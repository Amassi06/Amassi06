...
REPORT_MD = """# Rapport d'activité - 3ème année
...
def write_markdown() -> Path:
    path = OUT_DIR / "rapport_activite_aidel_massinissa.md"
...
def build_docx() -> Path:
    ...
    path = OUT_DIR / "rapport_activite_aidel_massinissa.docx"
    doc.save(path)
    return path
...
def build_pdf() -> Path:
    ...
    for line in iter_lines():
        ...
        if line.startswith("# "):
            ...
            story.append(Paragraph(clean_inline(line[2:]), normal, bulletText="•"))
        elif line.startswith("|"):
            story.append(Paragraph(clean_inline(line), normal))
        elif on_cover:
            story.append(Paragraph(clean_inline(line).replace("  ", "<br/>"), center))
        else:
            story.append(Paragraph(clean_inline(line), normal))

    path = OUT_DIR / "rapport_activite_aidel_massinissa.pdf"
